...
public class GatewayService {
    ...
    public GatewayResponse processPayment(GatewayRequest request) {
        ...
        log.info(
            "[{}] 🌍 Initiating gateway call for transaction {}",
            correlationId,
        ...
        try {
            ...
            log.info(
                "[{}] ✅ Gateway SUCCESS - Ref={} | Duration={}ms",
                correlationId,
                reference,
                durationMs
            );
    ...
    @CircuitBreaker(
        name = SERVICE_NAME,
        fallbackMethod = "fallbackWithdrawalResponse"
    )
    public GatewayResponse processWithdrawal(GatewayRequest request) {
        String correlationId = MDC.get("correlationId");
        long start = System.nanoTime();
        log.info(
            "[{}] 🌍 Initiating gateway withdrawal for transaction {}",
            correlationId,
            request.getTransactionId()
        );
        ...
        try {
            ...
            if (simulateFailure && random.nextInt(10) < 2) {
                throw new GatewayException(
                    "Simulated gateway withdrawal failure"
                );
            }
            ...
            long durationMs = (System.nanoTime() - start) / 1_000_000;
            log.info(
                "[{}] ✅ Gateway WITHDRAWAL SUCCESS - Ref={} | Duration={}ms",
                correlationId,
                reference,
                durationMs
            );
    ...
    // fallback for processWithdrawal
    public GatewayResponse fallbackWithdrawalResponse(
        GatewayRequest request,
        Throwable throwable
    ) {
        String correlationId = MDC.get("correlationId");
        log.warn(
            "[{}] ⚠️ Gateway withdrawal fallback for txn {} | Error: {} - {}",
            correlationId,
            request.getTransactionId(),
        ...
        return GatewayResponse.builder()
            ...
            .message(
                "Gateway temporarily unavailable: " + throwable.getMessage()
            )
            .build();
    }

    // fallback for processPayment
    public GatewayResponse fallbackResponse(
        GatewayRequest request,
        Throwable throwable
    ) {
        String correlationId = MDC.get("correlationId");
        ...
        log.warn(
            "[{}] ⚠️ Gateway fallback for txn {} | Error: {} - {}",
            correlationId,
            request.getTransactionId(),
            throwable.getClass().getSimpleName(),
            throwable.getMessage()
