...
public class WithdrawalService {
    ...
    public void withdraw(String authenticatedUserId, WithdrawRequest request) {
        try {
            ...
            var user = userRepository
                ...
                .orElseThrow(() ->
                    new PppsException(HttpStatus.NOT_FOUND, "User not found")
                );
            ...
            if (!walletIdToUse.equals(userWallet.getId())) {
                throw new PppsException(
                    ...
                    "Cannot withdraw from wallet you don't own"
                );
            }

            if (
                request.getSecurePin() == null ||
                request.getSecurePin().trim().isEmpty()
            ) {
                throw new PppsException(
                    HttpStatus.BAD_REQUEST,
                    "PIN is required for withdrawal"
                );
            }
            ...
            Wallet walletLocked = walletRepository.findByIdWithLock(
                walletIdToUse
            );
            if (walletLocked == null) throw new WalletNotFoundException(
                "Wallet not found (locked)"
            );
            ...
            if (fee.compareTo(BigDecimal.ZERO) > 0) {
                if (platformWalletId == null) {
                    throw new PppsException(
                        ...
                        "Platform wallet not configured to collect fees"
                    );
                }
                ...
                if (platformWallet == null) {
                    throw new PppsException(
                        HttpStatus.INTERNAL_SERVER_ERROR,
                        "Platform wallet not found to collect fees"
                    );
                }
            }
            ...
            walletLocked.setBalance(
                walletLocked.getBalance().subtract(totalDebit)
            );
            if (platformWallet != null) {
                platformWallet.setBalance(platformWallet.getBalance().add(fee));
            }
            walletRepository.saveAndFlush(walletLocked);
            if (platformWallet != null) walletRepository.saveAndFlush(
                platformWallet
            );
            ...
            // DEBIT the user wallet — ledger event (written async by LedgerEventConsumer)
            ...
            // FEE_REVENUE to platform wallet (only if fee > 0)
            if (
                fee.compareTo(BigDecimal.ZERO) > 0 && platformWalletId != null
            ) {
            ...
            GatewayResponse gatewayResponse = gatewayService.processWithdrawal(
                gatewayRequest
            );
            ...
            if ("SUCCESS".equalsIgnoreCase(gatewayResponse.getStatus())) {
                ...
                TransactionSynchronizationManager.registerSynchronization(
                    new TransactionSynchronization() {
                        ...
                        public void afterCommit() {
                            ...
                            log.info(
                                "📤 Kafka WithdrawalCompletedEvent published for withdrawal tx {}",
                                finalTx.getId()
                            );
                        }
                    }
                ...
                walletLocked.setBalance(
                    walletLocked.getBalance().add(totalDebit)
                );
                walletRepository.saveAndFlush(walletLocked);
                if (platformWallet != null) {
                    platformWallet.setBalance(
                        platformWallet.getBalance().subtract(fee)
                    );
    ...
    private boolean verifyPin(String providedPin, String hashedPin) {
        if (hashedPin == null) throw new PppsException(
            HttpStatus.BAD_REQUEST,
            "User PIN not set"
        );
